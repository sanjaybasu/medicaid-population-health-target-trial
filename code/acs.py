...
core=coredb("prod")
# pull PQI composite + acute for the ever-activated panel
panel=query(core,"""SELECT person_id, months_since_zero_date ms, enrolled_days,
  (pqi_01+pqi_03+pqi_05+pqi_07+pqi_08+pqi_11+pqi_12+pqi_14+pqi_15+pqi_16) pqi
...
PQ=mat("pqi");D=mat("enrolled_days");cols=list(range(-27,13));idx=PQ.index
def wsum(M,lo,hi):
...
for s in range(-12,1):
    ...
    if plo>phi: continue
    d=pd.DataFrame({"person_id":idx,"s":s,"treat":treat,"b":wsum(PQ,s-12,s-7).values,
      "mm_b":(wsum(D,s-12,s-7)/30.44).values,"p":wsum(PQ,plo,phi).values,"mm_p":(wsum(D,plo,phi)/30.44).values,
      "obs":(D[s]>0).values})
...
lm["base_rate"]=1000*lm.b/lm.mm_b.clip(lower=0.1)
...
t=lm[lm.treat==1]
print(f"### AHRQ PQI (ambulatory-care-sensitive hospitalization), n_treated={int(lm.treat.sum())}")
print(f"  corrected RR = {np.exp(b):.3f} ({np.exp(b-1.96*se):.3f}-{np.exp(b+1.96*se):.3f}) [{100*(1-np.exp(b)):.0f}% reduction]")
print(f"  treated post PQI rate = {1000*t.p.sum()/t.mm_p.sum():.1f}/1000mm")
# ED classification feasibility
cc=query(core,"""SELECT column_name FROM information_schema.columns WHERE table_schema='dbt'
  AND table_name='outcomes_with_enrollment__months_since'
  AND (column_name ILIKE '%ed%' OR column_name ILIKE '%emerg%' OR column_name ILIKE '%nyu%' OR column_name ILIKE '%avoid%' OR column_name ILIKE '%ambulatory%' OR column_name ILIKE '%treat%release%')""")
print("\nED-classification columns in panel:", list(cc.column_name))
